<?	
	...
	//$imgurl="./img/".$fileName;
	$imgurl="http://iop21727113.cafe24.com/freeadmin/pf/".$upload_filename;
	$no=$_GET['no'];
	$g=$_GET['g'];
	if(!$no)
		echo "<script>location.href=\"member.php?url=$imgurl&g=$g\"</script>";
	else{
		include ("dblib.php");
		$conn=mysqli_connect($db_host,$db_user,$db_passwd,$db_name);
		if($conn)echo "error!";
		$result=mysqli_query($conn,"update i set pf='$imgurl' where id=$no");
		echo "<script>location.href=\"member.php?g=$g\"</script>";
	}
?>